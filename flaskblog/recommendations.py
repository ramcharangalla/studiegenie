...
import os
#from flaskblog.create_notes import get_notes_df,get_interactions_df
...
def get_interactions_df():
	interactions_df = pd.read_csv(interactions_f)
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left',
                                                          left_on = note_id,
                                                          right_on = note_id)[['eventStrength', note_id, content_col,notes_userid]]
...
class ContentBasedRecommender:
    ...
    def recommend_items(self, user_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left',
                                                          left_on = note_id,
                                                          right_on = note_id)[['recStrength', note_id,content_col,notes_userid,'title','timestamp']]
...
# 		# mynote = RecNote(row[note_id],row[content_col],row[notes_userid],row['title'],row['timestamp'],row[names[np.random.randint(0,5)]])
# 		print(row[note_id])
# 		#recs.append(mynote)
# 	print(recs)
